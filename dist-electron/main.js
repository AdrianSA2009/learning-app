...
var require_combined_stream = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	util$1.inherits(CombinedStream, Stream$1);
...
//#endregion
//#region electron/moodleAPI.js
var require_moodleAPI = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	require_form_data();
	var MOODLE_URL = "https://learning-if.polibatam.ac.id";
	var API_ENDPOINT = `${MOODLE_URL}/webservice/rest/server.php`;
	var TOKEN_ENDPOINT = `${MOODLE_URL}/login/token.php`;
	var UPLOAD_ENDPOINT = `${MOODLE_URL}/webservice/upload.php`;
	var SERVICE_NAME = "moodle_mobile_app";
	async function callMoodle(token, wsfunction, params = {}) {
		...
		const text = await (await fetch(API_ENDPOINT, {
			method: "POST",
			body
		})).text();
		if (!text || text.trim() === "") throw new Error(`Response kosong dari server untuk ${wsfunction}`);
		let data;
		try {
	...
	async function loginMoodle(username, password) {
		...
		const data = await (await fetch(`${TOKEN_ENDPOINT}?${params}`)).json();
	...
	async function validateToken(token) {
		try {
			return {
				valid: true,
				data: await callMoodle(token, "core_webservice_get_site_info")
			};
		} catch (error) {
			return {
				valid: false,
	...
	async function getMyCourses(token, userId) {
		try {
			...
			if (error.message && error.message.toLowerCase().includes("token")) throw new Error("TOKEN_EXPIRED");
	...
	async function getAssignments(token, courseIds) {
		...
		try {
			return (await callMoodle(token, "mod_assign_get_assignments", { courseids: courseIds })).courses || [];
		} catch (error) {
			if (error.message && error.message.toLowerCase().includes("token")) throw new Error("TOKEN_EXPIRED");
			throw error;
		}
	...
	async function getMySubmissions(token, assignmentIds, userId) {
		...
		const results = [];
		for (const id of validIds) try {
			const status = (await callMoodle(token, "mod_assign_get_submission_status", {
				assignid: id,
				userid: userId
			...
			if (e.message && e.message.toLowerCase().includes("token")) throw new Error("TOKEN_EXPIRED");
			results.push({
				assignmentid: id,
	...
	async function getSubmissionStatus(token, assignmentId, userId) {
		try {
			const lastAttempt = (await callMoodle(token, "mod_assign_get_submission_status", {
				assignid: assignmentId,
				userid: userId
			})).lastattempt;
			let isSubmitted = false;
			if (lastAttempt && lastAttempt.submission) {
				if (lastAttempt.submission.status === "submitted") isSubmitted = true;
			}
			return { isSubmitted };
		} catch (error) {
	...
	async function uploadFileToDraft(token, filename, fileBuffer, mimeType) {
		...
		if (!Array.isArray(data) || !data[0]?.itemid) throw new Error("Gagal upload file ke Moodle: " + JSON.stringify(data));
	...
	function timeLeft(dueDate) {
		...
		const diff = dueDate * 1e3 - Date.now();
		if (diff <= 0) return "Sudah lewat";
		const days = Math.floor(diff / (1e3 * 60 * 60 * 24));
		const hours = Math.floor(diff % (1e3 * 60 * 60 * 24) / (1e3 * 60 * 60));
		if (days > 0) return `${days} hari ${hours} jam lagi`;
		if (hours > 0) return `${hours} jam lagi`;
		return `${Math.floor(diff % (1e3 * 60 * 60) / (1e3 * 60))} menit lagi`;
	}
	function shortCourseName(fullname) {
		if (!fullname) return "Mata Kuliah";
		return fullname.replace(/\[.*?\]/g, "").replace(/\(.*?\)/g, "").replace(/\d{4}\/\d{4}/g, "").trim().substring(0, 40);
	}
	function isExpired(dueDate) {
		if (!dueDate) return false;
		return /* @__PURE__ */ new Date(dueDate * 1e3) < /* @__PURE__ */ new Date();
	}
	...
	function formatDate(timestamp) {
		...
		return (/* @__PURE__ */ new Date(timestamp * 1e3)).toLocaleDateString("id-ID", {
			day: "numeric",
			month: "long",
			year: "numeric",
			hour: "2-digit",
...
var fs = require("fs");
var { loginMoodle, validateToken, getSiteInfo, getMyCourses, getAssignments, getMySubmissions, timeLeft, shortCourseName, getSubmissionStatus, isExpired, filterAssignmentByClass, formatDate, uploadFileToDraft, submitAssignmentFile, submitAssignmentText, getCourseContents } = require_moodleAPI();
var SESSION_FILE = path.join(app.getPath("userData"), "sessions.json");
var FILTER_FILE = path.join(app.getPath("userData"), "courseFilter.json");
...
function loadFilter(userId) {
	try {
		if (fs.existsSync(FILTER_FILE)) return JSON.parse(fs.readFileSync(FILTER_FILE, "utf8"))[userId] ?? null;
	} catch (err) {
...
function saveFilter(userId, filterData) {
	try {
		let all = {};
		if (fs.existsSync(FILTER_FILE)) all = JSON.parse(fs.readFileSync(FILTER_FILE, "utf8"));
...
app.whenReady().then(() => {
	...
	ipcMain.handle("moodle:getCourses", async () => {
		...
		try {
			if (!(await validateToken(currentSession.token)).valid) {
				clearCurrentSession();
				return {
					success: false,
	...
	ipcMain.handle("moodle:getFilter", async () => {
		...
		return {
			success: true,
			data: loadFilter(currentSession.userId)
		};
	});
	ipcMain.handle("moodle:saveFilter", async (event, filterData) => {
		...
		if (filterData && typeof filterData === "object" && !Array.isArray(filterData)) saveFilter(currentSession.userId, filterData);
		else saveFilter(currentSession.userId, { enabledCourseIds: filterData });
		return { success: true };
	});
	ipcMain.handle("moodle:getStatus", async () => {
		...
		try {
			...
			if (savedFilterRaw && savedFilterRaw.enabledCourseIds && typeof savedFilterRaw.enabledCourseIds === "object" && !Array.isArray(savedFilterRaw.enabledCourseIds)) savedFilter = savedFilterRaw.enabledCourseIds;
			...
			if (Array.isArray(assignmentData)) assignmentData.forEach((course) => {
				if (course.assignments && Array.isArray(course.assignments)) course.assignments.forEach((a) => {
					...
					if (shouldInclude) allAssignments.push({
						...a,
			...
			const now = Date.now();
			const tenDaysInMs = 14400 * 60 * 1e3;
			const relevantTasks = allAssignments.filter((a) => {
				if (!a.duedate) return false;
				return a.duedate * 1e3 > now - tenDaysInMs;
			});
			const assignIds = relevantTasks.map((a) => a.id).filter((id) => id && Number.isInteger(id));
			...
			const statusMap = {};
			if (Array.isArray(submissionData)) submissionData.forEach((sd) => {
				if (sd.submissions?.length > 0) statusMap[sd.assignmentid] = sd.submissions[0].status;
			});
			...
			const urgent = belum.filter((a) => {
				if (!a.duedate) return false;
				return a.duedate * 1e3 - now < 1440 * 60 * 1e3;
			});
			...
			return {
				...
				data: {
					...
					urgentTasks: urgent.map((a) => ({
						...
						isCritical: a.duedate * 1e3 - now < 720 * 60 * 1e3
					})),
			...
			if (error.message === "TOKEN_EXPIRED" || error.message.includes("Can't find data record") || error.message.toLowerCase().includes("invalidtoken")) {
				clearCurrentSession();
				return {
					success: false,
					error: "Sesi Moodle sudah expired. Silakan login ulang.",
	...
	ipcMain.handle("moodle:submitText", async (event, assignmentId, text) => {
		...
		try {
			...
			return {
				success: true,
				data: await submitAssignmentText(currentSession.token, assignmentId, text)
			};
		} catch (error) {
			return {
				success: false,
	...
	ipcMain.handle("moodle:submitFile", async (event, assignmentId, filePath) => {
		...
		try {
			...
			const draftItemId = await uploadFileToDraft(currentSession.token, filename, fileBuffer, "application/octet-stream");
			return {
				success: true,
				data: await submitAssignmentFile(currentSession.token, assignmentId, draftItemId)
			};
		} catch (error) {
			return {
				success: false,
	...
	ipcMain.handle("moodle:getCourseDetail", async (event, courseId) => {
		...
		try {
			...
			if (Array.isArray(assignmentData) && assignmentData.length > 0) {
				...
				if (courseObj.assignments && Array.isArray(courseObj.assignments)) courseObj.assignments.forEach((a) => {
			...
			if (keywordsStr && keywordsStr.trim()) {
				const keywords = keywordsStr.split(",").map((k) => k.trim().toLowerCase()).filter(Boolean);
				if (keywords.length > 0) filteredAssignments = courseAssignments.filter((a) => {
					const assignmentName = a.name.toLowerCase();
			...
			if (assignIds.length > 0) {
				const submissionData = await getMySubmissions(currentSession.token, assignIds, currentSession.userId);
				if (Array.isArray(submissionData)) submissionData.forEach((sd) => {
					if (sd.submissions?.length > 0) submissionStatusMap[sd.assignmentid] = sd.submissions[0].status;
				});
			}
			...
			const materials = [];
			if (Array.isArray(contents)) contents.forEach((section) => {
				if (section.modules && Array.isArray(section.modules)) section.modules.forEach((mod) => {
					if (mod.modname === "resource" && mod.contents && Array.isArray(mod.contents)) mod.contents.forEach((file) => {
						const name = file.filename || "";
						const ext = name.split(".").pop().toLowerCase();
						if (ext === "pdf" || ext === "docx" || ext === "doc") materials.push({
							name,
							url: file.fileurl,
							type: ext,
	...
	ipcMain.handle("dialog:openMaterial", async (event, fileUrl) => {
		...
		const authenticatedUrl = `${fileUrl}${fileUrl.includes("?") ? "&" : "?"}token=${currentSession.token}`;
